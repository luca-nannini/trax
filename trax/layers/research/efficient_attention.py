...
class LSHFF(base.Layer):
  ...
  def forward(self, x):
    ...
    x_buckets, _ = hash_vecs(x, self._n_buckets, self._n_hashes, rng1)
    ...
    w1_buckets, _ = hash_vecs(w1, self._n_buckets, self._n_hashes, rng2)
...
class SparseFF(base.Layer):
  ...
  def forward(self, x):
    ...
    if self._mode == 'train':
      ...
      w = np.reshape(w, [-1, batch_size, self._d1])
      mid = np.einsum('ai,iaj->aj', x, w)
      relu = np.where(mid <= 0, np.zeros_like(mid), mid)
  ...
  def init_weights_and_state(self, input_signature):
    ...
    w1 = self._kernel_initializer(shape_w1, rng_w1)
    w1 = np.reshape(w1, (d_model, self._d1, self._d2))
    w2 = self._kernel_initializer(shape_w2, rng_w2)
